...
for (clusterType in c("complete","average","mcquitty", "ward.D")){ 
  ...
  for (merge_dvg in c( "_nomerge_dvgPartialNone")){
    ...
    saveHeatmap(hm, file = sprintf("C:\\Users\\chels\\OneDrive\\Documents\\Pryzbyl_Lab\\Exploratory_Analysis\\Patient5\\CNV\\Heatmap\\clustvisHeatmap_CNV_%s_20DTF_NOrowCenter_NOscalling_%s_Patient5_%s_clustvis_%s.pdf",clsuterType, merge_dvg,Date, Package_version))
    
    #reading files for patient 8 
    file = sprintf("C:\\Users\\chels\\OneDrive\\Documents\\Pryzbyl_Lab\\CNV\\clustvis_format\\clustvisPatient8_CNVs_filtered%s.csv", merge_dvg)
    imp = importData(file, nbrRowAnnos = 0, nbrColAnnos = 5)
    proc = processData(imp, rowScaling = "none", rowCentering = FALSE)
    ...
    hm = generateHeatmap(proc, clustDistRows = NA, clustMethodRows = "ward.D", clustDistCols = "euclidean", 
                         clustMethodCols = clusterType, showRownames= F, colorAnnoCol = c("Type", "Mutation","Year"),legendColorScheme =  "Set2",colorRangeMax = 0.5, colorRangeMin = -0.5)
    saveHeatmap(hm, file = sprintf("C:\\Users\\chels\\OneDrive\\Documents\\Pryzbyl_Lab\\Exploratory_Analysis\\Patient8\\CNV\\Heatmap\\clustvisHeatmap_CNV_%s_20DTF_NOrowCenter_NOscalling_%s_Patient8_%s_clustvis_%s.pdf",clsuterType, merge_dvg,Date, Package_version))
  }
}
...
for (clsuterType in c("complete","average","mcquitty", "ward.D")){
  ...
  for (merge_dvg in c("_nomerge_dvgNotConsidered")){
    ...
    file = sprintf("C:\\Users\\chels\\OneDrive\\Documents\\Pryzbyl_Lab\\CNV\\clustvis_format\\clustvisPatient1_CNVs_filtered%s.csv", merge_dvg)
    ...
    saveHeatmap(hm, file = sprintf("C:\\Users\\chels\\OneDrive\\Documents\\Pryzbyl_Lab\\Exploratory_Analysis\\Patient1\\CNV\\Heatmap\\clustvisHeatmap_CNV_%s_20DTF_NOrowCenter_NOscalling_%s_Patient1_%s_clustvis_%s.pdf",clsuterType, merge_dvg,Date, Package_version))
  }
}
