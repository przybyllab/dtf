  ...
  for (Patient_ID in c(1,5,8)){ 
    ...
    for (clsuterType in c("complete","average","mcquitty", "ward.D")){
      ...
      for (topxvar in  c("top_1percent")){
        ...
        #reading files 
        file = sprintf("[path to clustvis file]\\%s_beta_value_var_20DTFsamples_Patient_%d.csv", topxvar, Patient_ID)
        ...
        pca = generatePCA(proc, pcx = 1, pcy = 2, switchDirX = FALSE,
                          ...
                          maxShapeLevels = 62)
        savePCA(pca, file = sprintf("[Path to save PCA]\\clustvis_PCA_Euclidean_Ward_20DTFsamples_NOrowCentering_%s_scalling_%s_beta_matrix_Patient%d_%s_clustvisVersion_%s.pdf",Patient_ID,scalling_type, topxvar,Patient_ID,Date, Package_version))
        ...
        saveHeatmap(hm, file = sprintf("[Path to heatmaps]\\clustvisHeatmap_Euclidean_%s_20DTFsamples_NOrowcentering_%s_scalling_%s_beta_matrix_Patient%d_%s_clustvisVersion_%s.pdf", Patient_ID, clsuterType,scalling_type, topxvar,Patient_ID,Date, Package_version))
